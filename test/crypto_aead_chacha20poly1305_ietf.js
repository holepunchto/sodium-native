...
test('ported from libsodium', function (t) {
  ...
  const exp2 = Buffer.from([
    ...
    0xfa, 0xb3, 0x24, 0xe4, 0xfa, 0xd6, 0x75, 0x94, 0x55, 0x85, 0x80, 0x8b, 0x48, 0x31, 0xd7, 0xbc,
    0x3f, 0xf4, 0xde, 0xf0, 0x8e, 0x4b, 0x7a, 0x9d, 0xe5, 0x76, 0xd2, 0x65, 0x86, 0xce, 0xc6, 0x4b,
    0x61, 0x16, 0x6a, 0x23, 0xa4, 0x68, 0x1f, 0xd5, 0x94, 0x56, 0xae, 0xa1, 0xd2, 0x9f, 0x82, 0x47,
    0x72, 0x16
  ])
  exp2.secure = true
...
test('different keys', function (t) {
  ...
  t.exception.all((_) =>
    sodium.crypto_aead_chacha20poly1305_ietf_decrypt(m1, null, c1, null, nonce, key2)
  )
  t.exception.all((_) =>
    sodium.crypto_aead_chacha20poly1305_ietf_decrypt(m2, null, c2, null, nonce, key1)
  )

  t.is(
    sodium.crypto_aead_chacha20poly1305_ietf_decrypt(m1, null, c1, null, nonce, key1),
    m.byteLength
  )
  t.ok(m.equals(m1))
  t.is(
    sodium.crypto_aead_chacha20poly1305_ietf_decrypt(m2, null, c2, null, nonce, key2),
    m.byteLength
  )
  t.ok(m.equals(m2))
})
...
test('different nonce', function (t) {
  ...
  t.exception.all((_) =>
    sodium.crypto_aead_chacha20poly1305_ietf_decrypt(m1, null, c1, null, n2, key)
  )
  t.exception.all((_) =>
    sodium.crypto_aead_chacha20poly1305_ietf_decrypt(m2, null, c2, null, n1, key)
  )
...
test('detached -> non-detached', function (t) {
  ...
  t.is(
    sodium.crypto_aead_chacha20poly1305_ietf_encrypt_detached(c, mac, m, null, null, nonce, key),
    mac.byteLength
  )
